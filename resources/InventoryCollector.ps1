...
Try
  {    
    ...
    $sys = Get-WmiObject Win32_ComputerSystem | select Name, Domain, Manufacturer, Model, @{Name="DomainRole"; Expression = {$domainRoles["$($_.DomainRole)"]}}, SystemType, NumberofLogicalProcessors,NumberofProcessors, OEMStringArray
    ...
    if (-not ($sys2.IsVirtual))
        {
            ...
            #$totalDIMMS = $colSlots.memorydevices
    ...
    $fqdn=$sys.name+'.'+$sys.domain
    $t= c:\windows\system32\nslookup.exe $fqdn
    $sysIP=($t[4].Split(' '))[2]

    #$sysIP = Get-SystemIPAddress
    ...
    $SystemInfo += New-Object PSObject -Property @{
                                                    ...
                                                    systemFQDN = $sys.Name + "." + $sys.Domain
 ...
 Catch {LogEvent 0 "an error occured while trying to collect system information: $_"}
 }
