...
namespace ServersInventoryAPI
{
    public static class Utility
    {
        ...
        public static bool InsertNetworkStats(List<NetworkStat> networkstats, Int32 serverId)
        {
            bool result = false;
            try
            {
                using (var connection = ConnectionManager.Create())
                {
                    var netstat = new DynamicParameters();
                    foreach (var ns in networkstats)
                    {
                        netstat.Add("@server_id", serverId);
                        netstat.Add("@PID", Utility.CheckIntValue(ns.PID));
                        netstat.Add("@ProcessName", ns.ProcessName);
                        netstat.Add("@Protocol", ns.Protocol);
                        netstat.Add("@LocalAddress", ns.LocalAddress);
                        netstat.Add("@LocalPort", Utility.CheckIntValue(ns.LocalPort));
                        netstat.Add("@RemoteAddress", ns.RemoteAddress);
                        netstat.Add("@RemotePort", Utility.CheckIntValue(ns.RemotePort));
                        netstat.Add("@State", ns.State);
                        connection.Execute("usp_ins_NetworkStatistic", netstat, null, null, System.Data.CommandType.StoredProcedure);
                    }
                }
                result = true;
            }
            ...
            {
                LoggingManager.Instance.GetLogger.Error("Error executing stored proc <usp_ins_NetworkStatistic>", ex);
            }
            return result;
        }
        ...
        public static bool InsertSQLJobHistory(List<SQL_JobHistory> sqljobhistory)
        {
            bool result = false;
            try
            {
                using (var connection = ConnectionManager.Create())
                {
                    var sqlJH = new DynamicParameters();
                    foreach (var job in sqljobhistory)
                    {
                        sqlJH.Add("@job_server", job.job_server);
                        sqlJH.Add("@instance_name", job.instance_name);
                        sqlJH.Add("@instance_id", Utility.CheckIntValue(job.instance_id));
                        sqlJH.Add("@job_id", job.job_id);
                        sqlJH.Add("@job_name", job.job_name);
                        sqlJH.Add("@step_id", Utility.CheckIntValue(job.step_id));
                        sqlJH.Add("@step_name", job.step_name);
                        sqlJH.Add("@sql_message_id", Utility.CheckIntValue(job.sql_message_id));
                        sqlJH.Add("@sql_severity", Utility.CheckIntValue(job.sql_severity));
                        sqlJH.Add("@job_step_message", job.job_step_message);
                        sqlJH.Add("@run_status", Utility.CheckIntValue(job.run_status));
                        sqlJH.Add("@run_date", Utility.CheckIntValue(job.run_time));
                        sqlJH.Add("@run_time", Utility.CheckIntValue(job.run_time));
                        sqlJH.Add("@run_duration", Utility.CheckIntValue(job.run_duration));
                        sqlJH.Add("@operator_emailed", job.operator_emailed);
                        sqlJH.Add("@operator_netsent", job.operator_netsent);
                        sqlJH.Add("@operator_paged", job.operator_paged);
                        sqlJH.Add("@retries_attempted", Utility.CheckIntValue(job.retries_attempted));
                        sqlJH.Add("@job_start_datetime", Utility.CheckDateTimeValue(job.job_start_datetime));

                        connection.Execute("usp_ins_SQL_JobHistory", sqlJH, null, null, System.Data.CommandType.StoredProcedure);
                    }
                }
                result = true;
            }
            ...
            {
                LoggingManager.Instance.GetLogger.Error("Error executing stored proc <usp_ins_SQL_JobHistory>", ex);
            }
            return result;
        }
        
        public static bool InsertSQLDatabaseFileInfo(List<SQL_DatabaseFileInfo> sqldatabasefileinfo)
        {
            bool result = false;
            try
            {
                using (var connection = ConnectionManager.Create())
                {
                    var sqlDF = new DynamicParameters();
                    foreach (var df in sqldatabasefileinfo)
                    {
                        sqlDF.Add("@ServerName", df.servername);
                        sqlDF.Add("@InstanceName", df.instancename);
                        sqlDF.Add("@database_id", Utility.CheckIntValue(df.database_id));
                        sqlDF.Add("@db_file_id", Utility.CheckIntValue(df.db_file_id));
                        sqlDF.Add("@db_file_type", df.db_file_type);
                        sqlDF.Add("@database_name", df.database_name);
                        sqlDF.Add("@db_file_name", df.db_file_name);
                        sqlDF.Add("@db_file_size", Utility.CheckIntValue(df.db_file_size));
                        sqlDF.Add("@db_file_used", Utility.CheckIntValue(df.db_file_used));
                        sqlDF.Add("@stat_time", Utility.CheckDateTimeValue(df.stat_time));
                        sqlDF.Add("@autogrow", Utility.CheckIntValue(df.autogrow));
                        sqlDF.Add("@autogrow_pct", df.autogrow_pct);

                        connection.Execute("usp_ins_SQL_DBFileInformation", sqlDF, null, null, System.Data.CommandType.StoredProcedure);
                    }
                }
                result = true;
            }
            ...
            {
                LoggingManager.Instance.GetLogger.Error("Error executing stored proc <usp_ins_SQL_DBFileInformation>", ex);
            }
            return result;
        }

        public static bool InsertSQLInstanceStats(List<SQL_InstanceStat> sqlinstancestats)
        {
            bool result = false;
            try
            {
                using (var connection = ConnectionManager.Create())
                {
                    var sqlIS = new DynamicParameters();
                    foreach (var stat in sqlinstancestats)
                    {
                        sqlIS.Add("@ServerName", stat.servername);
                        sqlIS.Add("@InstanceName", stat.instancename);
                        sqlIS.Add("@StatDate",  Utility.CheckDateTimeValue(stat.statdate));
                        sqlIS.Add("@Ident_Type_ID", Utility.CheckIntValue(stat.ident_type_id));
                        sqlIS.Add("@StatValue", stat.statvalue);

                        connection.Execute("usp_ins_SQL_InstanceStat", sqlIS, null, null, System.Data.CommandType.StoredProcedure);
                    }
                }
                result = true;
            }
            ...
            {
                LoggingManager.Instance.GetLogger.Error("Error executing stored proc <usp_ins_SQL_InstanceStat>", ex);
            }
            return result;
        }

        public static bool InsertSQLCPUStats(List<SQL_CPUStat> sqlcpustats)
        {
            bool result = false;
            try
            {
                using (var connection = ConnectionManager.Create())
                {
                    var sqlCPUS = new DynamicParameters();
                    foreach (var stat in sqlcpustats)
                    {
                        sqlCPUS.Add("@ServerName", stat.servername);
                        sqlCPUS.Add("@InstanceName", stat.instancename);
                        sqlCPUS.Add("@StatTime", Utility.CheckDateTimeValue(stat.stattime));
                        sqlCPUS.Add("@CPUIdle", Utility.CheckIntValue(stat.cpuidle));
                        sqlCPUS.Add("@CPUSQL", Utility.CheckIntValue(stat.cpusql));
                        sqlCPUS.Add("@CPUOther", Utility.CheckIntValue(stat.cpuother));
                        sqlCPUS.Add("@EventTime", Utility.CheckDateTimeValue(stat.eventtime));

                        connection.Execute("usp_ins_SQL_CPUStat", sqlCPUS, null, null, System.Data.CommandType.StoredProcedure);
                    }
                }
                result = true;
            }
            ...
            {
                LoggingManager.Instance.GetLogger.Error("Error executing stored proc <usp_ins_SQL_CPUStat>", ex);
            }
            return result;
        }

        public static bool InsertSQLBackups(List<SQL_Backup> sqlbackups)
        {
            bool result = false;
            try
            {
                using (var connection = ConnectionManager.Create())
                {
                    var sqlB = new DynamicParameters();
                    foreach (var b in sqlbackups)
                    {
                        sqlB.Add("@ServerName", b.servername);
                        sqlB.Add("@InstanceName", b.instancename);
                        sqlB.Add("@DBName", b.dbname);
                        sqlB.Add("@StatDate", Utility.CheckDateTimeValue(b.statdate));
                        sqlB.Add("@Ident_Type_ID", Utility.CheckIntValue(b.ident_type_id));
                        sqlB.Add("@StatValue", b.statvalue);

                        connection.Execute("usp_ins_SQL_Backup", sqlB, null, null, System.Data.CommandType.StoredProcedure);
                    }
                }
                result = true;
            }
            ...
            {
                LoggingManager.Instance.GetLogger.Error("Error executing stored proc <usp_ins_SQL_Backup>", ex);
            }
            return result;
        }

        public static bool InsertSQLDBIOStats(List<SQL_DBIOStat> sqlbackups)
        {
            bool result = false;
            try
            {
                using (var connection = ConnectionManager.Create())
                {
                    var sqlDBIO = new DynamicParameters();
                    foreach (var b in sqlbackups)
                    {
                        sqlDBIO.Add("@ServerName", b.servername);
                        sqlDBIO.Add("@InstanceName", b.instancename);
                        sqlDBIO.Add("@database_name", b.database_name);
                        sqlDBIO.Add("@database_id", Utility.CheckIntValue(b.database_id));
                        sqlDBIO.Add("@read_wait_ms", Utility.CheckIntValue(b.read_wait_ms));
                        sqlDBIO.Add("@reads", Utility.CheckIntValue(b.reads));
                        sqlDBIO.Add("@write_wait_ms", Utility.CheckIntValue(b.write_wait_ms));
                        sqlDBIO.Add("@writes", Utility.CheckIntValue(b.writes)); 
                        sqlDBIO.Add("@stat_time", Utility.CheckDateTimeValue(b.stat_time));

                        connection.Execute("usp_ins_SQL_DBIOStat", sqlDBIO, null, null, System.Data.CommandType.StoredProcedure);
                    }
                }
                result = true;
            }
            ...
            {
                LoggingManager.Instance.GetLogger.Error("Error executing stored proc <usp_ins_SQL_DBIOStat>", ex);
            }
            return result;
        }


        public static bool InsertSQLDBRecordingSize(List<SQL_DBRecordingSize> sqldbrecordingsize)
        {
            bool result = false;
            try
            {
                using (var connection = ConnectionManager.Create())
                {
                    var sqlDBRS = new DynamicParameters();
                    foreach (var req in sqldbrecordingsize)
                    {
                        sqlDBRS.Add("@servername", req.servername);
                        sqlDBRS.Add("@instancename", req.instancename);
                        sqlDBRS.Add("@database_name", req.databasename);
                        sqlDBRS.Add("@database_id", Utility.CheckIntValue(req.databaseid));
                        sqlDBRS.Add("@totalrecordingsize", Utility.CheckIntValue(req.totalrecordingsize));
                        sqlDBRS.Add("@stat_date", Utility.CheckDateTimeValue(req.statdate));
                        
                        connection.Execute("usp_ins_SQL_RecordingSize", sqlDBRS, null, null, System.Data.CommandType.StoredProcedure);
                    }
                }
                result = true;
            }
            ...
            {
                LoggingManager.Instance.GetLogger.Error("Error executing stored proc <usp_ins_SQL_RecordingSize>", ex);
            }
            return result;
        }
    }
